...
exports.updateCart = async (req, res) => {
  ...
  try {
    ...
    if (checkResult.length > 0) {
      ...
      if (quantity > availableQuantity) {
        return res.status(400).send({ message: `Available Quantity  of this product is ${availableQuantity}` });
      }
      ...
      if (quantity > availableQuantity) {
        return res.status(400).send({ message: `You can only add up to ${availableQuantity} units for this product` });
      }
      ...
      if (insertResult.affectedRows > 0) {
        ...
        return res.status(400).send({ message: 'Failed to add product to cart' });
      }
    }
  } catch (err) {
    return res.status(500).send({ message: 'Error updating cart', error: err.message });
  }
};
